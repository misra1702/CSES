...
int main(){
	...
	if(rem==0){
		FOR(i,0,mid){
			if(i%2==0)	st1.insert(i+1);
			else st2.insert(i+1);
		}
		for(ll i = n-1;i>=mid;--i){
			ll j = n-1-i;
			if(j%2==0)	st1.insert(i+1);
			else st2.insert(i+1);
		}
	}
	else{
		mid++;
		set<ll> st3 = {mid/2,mid,mid/2+mid};
		st1.insert(mid/2);
		st1.insert(mid);
		st2.insert(mid+mid/2);
		ll prev = 0;
		FOR(i,1,mid){
			if(st3.find(i)!=st3.end())	continue;
			if(!prev)	st1.insert(i);
			else st2.insert(i);
			prev ^=1;
		}
		prev = 0;
