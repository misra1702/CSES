...
public:
	...
	void push(ll x){
		...
		t[2*x+1] += lazy[x];	lazy[2*x+1] += lazy[x];	mark[2*x+1] = 1;
		lazy[x] = 0;	mark[x] = 0;
	}
	
	void updateRange(ll ind,ll tl ,ll tr, ll l ,ll r ,ll val){
		if(l>r)	return;
		if(tl==l && tr==r){	t[ind] += val;	lazy[ind] += val;	mark[ind] = 1;	return ;}
	...
	ll query(ll ind ,ll tl ,ll tr, ll l ,ll r){
		...
		if(tl==l && tr==r){	return t[ind]+lazy[ind];}
		push(ind);
		ll tm = (tl+tr)/2;		
		return combine(query(2*ind,tl,tm,l,min(tm,tr)), query(2*ind+1,tm+1,tr,max(l,tm+1),tr));
	}
};
