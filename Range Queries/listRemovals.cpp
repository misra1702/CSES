...
public:
	...
	ll query(ll ind ,ll tl ,ll tr , ll x){
		// cout<<tl<<" "<<tr<<" "<<x<<" "<<t[ind]<<endl;
		if(tl == tr)
			return tl;
		ll tm = (tl+tr)/2;
		if(t[2*ind]>=x)
			return query(2*ind,tl,tm,x);
		else 
			return query(2*ind+1,tm+1,tr,x-t[2*ind]);
	}
};
class FEN{
public: vll v;	ll n;
...
int main(){
	...
	FOR(i,0,n){
		ll x;	cin>>x;
		ll ind = tr.query(1,0,n-1,x);
		cout<<v[ind]<<" ";
		tr.update(1,0,n-1,ind,0);
	}
	cout<<endl;
