...
public:
	...
	void push(ll x){
		if(!mark[x])	return;	t[2*x] += lazy[x];	lazy[2*x] += lazy[x];	mark[2*x] =1;
...
int main(){
	...
	FOR(i,0,q){
		...
		if(x==1){
			...
			--pos;
			tri.update(1,0,n-1,pos,val+pos);
			trd.update(1,0,n-1,pos,val-pos);
		}
		else{
			ll pos;	cin>>pos;	--pos;
			ll a1 = tri.query(1,0,n-1,pos,n-1);
			a1 -= pos;
			ll a2 = trd.query(1,0,n-1,0,pos);
			a2 +=pos;
			cout<<min(a1,a2)<<endl;
		}
	}
